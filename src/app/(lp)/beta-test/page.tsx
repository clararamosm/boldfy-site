...
/**
 * /beta-test — Landing Page standalone do programa beta da Boldfy.
 *
 * Decisões de arquitetura:
 *  - Mora dentro do route group `(lp)` pra ficar isolada do header/footer
 *    globais do site (que são escondidos pelo `ConditionalChrome` quando
 *    a rota é uma LP).
 *  - Usa `<LpHeader />` e `<LpFooter />` minimalistas.
 *  - Simulador é o componente compartilhado `<RoiSimulator />` (mesma
 *    fonte de verdade que vai aparecer na home). Toda a lógica de CPM
 *    e preços vem de `src/lib/constants.ts`.
 *  - Pricing tiles e banner BETA usam helpers `getFullPricePerSeat` /
 *    `getBetaPricePerSeat` — quando a Clara virar o flag
 *    `BETA_PRICING_ENABLED` pra `false` no fim do programa, a página
 *    inteira passa a mostrar só preço cheio sem código duplicado.
 */
...
export default function BetaTestPage() {
  ...
  // Showcase: 5 telas da plataforma com rotações e absolute positioning.
  // Imagens vivem em /public/images/pitch/ — quando a plataforma mudar de
  // visual, basta substituir os PNGs mantendo os mesmos nomes de arquivo.
  const showcase = [
    {
      key: 'dashboard',
      title: t.betaTest.showcaseDashboardTitle,
      desc: t.betaTest.showcaseDashboardDesc,
      img: '/images/pitch/dashboard.png',
      // posição (em %) e rotação dentro do container do showcase
      style: { top: '0%', left: '4%', rotate: '-4deg', z: 10 },
    },
    {
      key: 'assistente-ia',
      title: t.betaTest.showcaseAssistantTitle,
      desc: t.betaTest.showcaseAssistantDesc,
      img: '/images/pitch/assistente-ia.png',
      style: { top: '6%', left: '38%', rotate: '3deg', z: 30 },
    },
    {
      key: 'rankings',
      title: t.betaTest.showcaseRankingsTitle,
      desc: t.betaTest.showcaseRankingsDesc,
      img: '/images/pitch/rankings.png',
      style: { top: '0%', left: '70%', rotate: '5deg', z: 20 },
    },
    {
      key: 'calendario',
      title: t.betaTest.showcaseCalendarTitle,
      desc: t.betaTest.showcaseCalendarDesc,
      img: '/images/pitch/calendario.png',
      style: { top: '52%', left: '14%', rotate: '4deg', z: 15 },
    },
    {
      key: 'trilhas',
      title: t.betaTest.showcaseTrailsTitle,
      desc: t.betaTest.showcaseTrailsDesc,
      img: '/images/pitch/trilhas.png',
      style: { top: '52%', left: '54%', rotate: '-3deg', z: 25 },
    },
  ];

  // Pricing tiles montados a partir das constants — preço cheio sempre,
  // preço beta condicional ao flag BETA_PRICING_ENABLED.
  const pricingTiles = [
    ...PRICING_TIERS.map((tier, i) => {
      const minSeats = i === 0 ? 5 : PRICING_TIERS[i - 1].maxSeats + 1;
      return {
        range: `${minSeats} – ${tier.maxSeats} seats`,
        fullPrice: tier.fullPrice,
        betaPrice: getBetaPricePerSeat(tier.maxSeats),
        featured: tier.maxSeats === 40, // tier 21-40 destacado
        enterprise: false,
      };
    }),
    {
  ...
  return (
    <>
      <LpHeader
        ...
        ctaLabel={t.betaTest.betaTesterButton}
        trackingSource="beta:lp-header"
      />

      <div className="mx-auto max-w-6xl px-6">

        {/* HERO — sem mockup placeholder */}
        ...
        {/* ROI SIMULATOR — componente compartilhado, idêntico à versão da home */}
        <div className="mb-6">
          <RoiSimulator />
        </div>
        ...
        {/* SHOWCASE — 5 telas da plataforma com rotações */}
        <section className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
            {showcase.map((s) => (
              <div key={s.key} className="bg-card border rounded-xl overflow-hidden">
                ...
                <div className="p-3">
                  <p className="text-[12px] font-bold text-accent-foreground">{s.title}</p>
          ...
          {/* Desktop (absolute positioned com rotações) */}
          <div className="hidden md:block relative w-full" style={{ paddingBottom: '78%' }}>
            <div className="absolute inset-0">
              {showcase.map((s) => (
                <div
                  key={s.key}
                  className="absolute w-[28%] transition-transform duration-300 hover:scale-105 hover:!rotate-0"
                  style={{
                    top: s.style.top,
                    left: s.style.left,
                    transform: `rotate(${s.style.rotate})`,
                    zIndex: s.style.z,
                  }}
                >
                  <div className="bg-card border rounded-xl overflow-hidden shadow-lg">
                    ...
                    <div className="p-2.5">
                      <p className="text-[11px] font-bold text-accent-foreground">{s.title}</p>
                      <p className="text-[9px] text-muted-foreground leading-snug">{s.desc}</p>
                    </div>
                  </div>
                </div>
        ...
        {/* BETA BANNER — texto novo: 1 mês grátis, -30% nos 5 seguintes, oferta até 15/05 */}
        ...
        {/* PRICING — montado a partir de PRICING_TIERS, beta condicional ao flag */}
          ...
          {pricingTiles.map((p) => (
            <div key={p.range} className={`bg-card rounded-xl p-4 text-center ${p.featured ? 'border-2 border-primary bg-secondary' : 'border'} ${p.enterprise ? 'bg-gradient-to-br from-[#1A0E2E] to-[#0F0A18] border-primary/30' : ''}`}>
              ...
              {p.enterprise ? (
                ...
                <>
                  <p className="text-[10px] text-muted-foreground line-through mb-0.5">R$ {p.fullPrice}</p>
                  <p className="font-headline text-xl font-black text-accent-foreground">R$ {p.betaPrice}</p>
                ...
                <>
                  <p className="font-headline text-xl font-black text-accent-foreground">R$ {p.fullPrice}</p>
                  <p className="text-[9px] text-muted-foreground">{t.betaTest.seatMonth}</p>
                </>
              )}
            </div>
        ...
        {/* FORM */}
        <section id={FORM_SECTION_ID} className="bg-card border rounded-xl p-6 md:p-8 mb-8 scroll-mt-20">
          {sent ? (
