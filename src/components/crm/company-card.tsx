/**
 * Company card — usado no kanban Empresas.
 *
 * Compact: nome, indústria/porte, count de pessoas linkadas, top lead score
 * agregado, próxima ação (se preenchida). "Parada há N dias" se updated_at é
 * antigo (>14d).
 */

import Link from 'next/link';
import type { CompanyWithStats } from '@/lib/crm-queries';
import { timeAgo } from '@/lib/crm-format';

type Props = { company: CompanyWithStats };
...
export function CompanyCard({ company }: Props) {
  const stale = daysSinceUpdate(company.updatedAt) > 14 &&
    (company.status === 'Em andamento' || company.status === 'Reunião marcada');

  return (
    <Link href={`/internal/crm/companies/${company.id}`} className="crm-company-card">
